...
class DataSciencePage extends React.Component {
  ...
  componentDidMount() {
    ...
    const svg = d3
      .select(`#${idName || "data-science-page-d3"}`)
      .append("svg")
      .attr("width", "100%")
      .attr("height", "100%")
  ...
  render() {
    const { style: propStyle, className, id, idName } = this.props
    const { activeContent } = this.state

    return (
      <Container
        style={{ ...propStyle, ...style.container }}
        className={className}
        id={id}
      >
        <Row>
          <Col id={idName || "data-science-page-d3"} xs={8} />
          <Col>{activeContent}</Col>
        </Row>
      </Container>
    )
  }
